...
**Backoffice SPA hygiene + infra fixes (2026-04-28):**
- **Reports SPA on `ReportView`**: `Sales` / `AbandonCart` / `RestockSubscriptions` migrated; `Transaction` keeps custom layout (tabs + KPI cards). `ReportView` extended with `showDateFilters` / `showStoreFilter` / `extraFilters` / `headerExtras` / `onReset` / `autoLoad` / `initialDate*`. `OperationalReportPage` deleted (orphan). `DeliverySlotReportPage` is now a styled "coming soon" placeholder. Transaction page no longer mocks data when `tenantId` is missing — clear error instead. `mapApiDetail` no longer fakes empty `brandCode`/`paymentMethod` (DTO doesn't carry them); filters still pass through to API. Default date range = last 7 days (was hard-coded `2026-04-01..18`).
- **Gateway path bug**: YARP `/gateway/v1/orders/{**catch-all}` strips `/gateway/v1/orders/` and prepends `/api/`. Routes at `/api/orders/...` therefore need `${GATEWAY.orders}/orders/...` from the SPA (double `orders` is correct). Fixed 6 paths in `reports/api/reportsApi.ts` (transactions/summary|details|ecommerce + sales/by-category|by-product|by-tenant).
- **OrderReportQueryStore cross-DB bug**: `GetTransactionSummaryAsync` + `GetEcommercePaymentsAsync` joined `"PaymentTransactions"` (Payment DB) over **Order DB** connection — pre-existing latent bug, surfaced by Reports SPA. Re-targeted to `OrderPayments` + `PaymentComponents` (DAI-722, in-DB). Now groups by `pc."Type"` (Voucher/LoyaltyPoints/GiftCard/Gateway), filtered to `State IN ('Captured','Authorized','Refunded')`. `COUNT(DISTINCT)::int` cast for Dapper Int32 mapping. Multi-tender orders correctly contribute one row per component.
- **Section context pattern**: all custom elements (`dcms-{orders|access|approval|estore|reports}-section.js`) must `await fetchXxxContext()` and pass `{tenantId, storeId, authToken}` as 2nd arg to `mount(host, ctx)`. Reports section was the only one missing this — fixed. Context endpoint: `GET /umbraco/dcms/api/estore/context` ([DcmsEstoreContextController.cs](src/backend/dCMS.Web/Controllers/DcmsEstoreContextController.cs)) reads `Dcms:Estore:{TenantId,StoreId}` and mints a JWT with all dCMS roles when `Dcms:Auth:JwtSigningKey` is set.
- **`dCMS.Web` Redis required**: `DcmsAccessComposer` registers `IConnectionMultiplexer` only when `ConnectionStrings:Redis` is set; `PermissionCache` declares `IConnectionMultiplexer?` (nullable) but DI `ValidateOnBuild` (default in Development) requires the type to be registered regardless. Added `ConnectionStrings:Redis=127.0.0.1:6379,abortConnect=false` to `appsettings.Development.json`. `abortConnect=false` lets the app boot when Redis is offline (PermissionCache falls back to memory-only).
- **Hangfire dashboard auth**: `/umbraco/dcms/hangfire` is mounted via `UmbracoPipelineFilter` so the backoffice cookie is decoded, but `IDashboardAuthorizationFilter` runs before the filter that populates `HttpContext.User`. Filter now uses `IBackOfficeSecurityAccessor.BackOfficeSecurity?.IsAuthenticated()` (canonical pattern, used by `DcmsBulkJobsController` + `RequirePermissionAttribute`) as primary check. Optional shared-key fallback via `Dcms:Hangfire:DashboardKey` accepts both `X-Dcms-Hangfire-Key` header and `?accessKey=` query param.
- **Bulk Jobs page removed**: `src/estore/pages/BulkJobsPage.tsx` deleted; "Bulk jobs" item gone from eStore nav (under Products). Replaced by **Background Jobs** card in `DashboardApp.tsx` (`span-12 span-lg-4`, sits next to Recent Orders), polls `listBulkJobs(20)` every 4s, shows progress bar + Cancel/Retry/Download actions, header link to Hangfire dashboard.
- **Orders pages template alignment**: `FailedOrdersPage` / `RefundCasesPage` / `ReturnsPage` refactored to match `OrderProcessingPage`'s header/`<select>` filter / Export / floating toast / dialog pattern. `ReturnsPage` moved from hand-rolled `<table>` to `DataTable` with `ColumnDef<ReturnDto>`.
- **Voucher.Api / Loyalty.Api `Program`** made `internal partial class` — `dCMS.Order.Tests` now project-references both for hold-expiry integration tests; `public Program` would have collided with `Order.Api`'s `Program` in `WebApplicationFactory<Program>` tests. `dCMS.Order.Tests.csproj` Dapper bumped 2.1.35 → 2.1.72 to align with Voucher/Loyalty (`NU1605` downgrade error otherwise).
