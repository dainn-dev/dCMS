...
export default defineConfig({
  ...
  css: {
    postcss: {
      plugins: [
        ...
        autoprefixer(),
        prefixer({
          prefix: ".dcmsApprovalSpa",
          exclude: [/^\.dcmsApprovalSpa/, /:root/, /^@/],
  ...
  build: {
    ...
    emptyOutDir: false,
    lib: {
      entry: "src/approval/index.tsx",
      formats: ["es"],
      fileName: () => "approval-spa.js",
    },
    cssCodeSplit: false,
    sourcemap: true,
    rollupOptions: {
      output: {
        ...
        assetFileNames: (assetInfo) => {
          if (assetInfo.name && assetInfo.name.endsWith(".css")) return "approval-spa.css";
          return "approval-spa.[ext]";
        },
      },
    },
  },
