...
type Props<T extends { id: string }> = {
  ...
  /** Hide the built-in date/store filters when the report doesn't need them. */
  showDateFilters?: boolean;
  showStoreFilter?: boolean;
  /** Extra filter inputs (rendered alongside date/store). Should be <ReportFilterField> children. */
...
export function ReportView<T extends { id: string }>({
  ...
  // Keep loadData stable in the search closure so external filter changes
  // (extraFilters captured in the caller's loadData closure) re-trigger
  ...
  const handleSearch = useCallback(async () => {
    ...
    setHasSearched(true);
    try {
      const next = await loadDataRef.current(filters);
      setRows(next);
    } finally {
      setLoading(false);
    }
  ...
  // Auto-load on mount (and when external loadData closure changes — caller
  // passes a memoised loadData that closes over its filter state).
  useEffect(() => {
    if (!autoLoad) return;
    void handleSearch();
  ...
  const handleReset = useCallback(() => {
    ...
    onReset?.();
  }, [initialDateFrom, initialDateTo, onReset]);
      ...
      <header className="flex shrink-0 flex-col gap-4 border-b border-outline-variant/15 bg-surface px-6 py-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 self-start md:self-center">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-outline-variant/30 bg-white px-4 py-2 text-xs font-bold text-on-surface shadow-sm hover:bg-surface-container-high disabled:pointer-events-none disabled:opacity-40"
