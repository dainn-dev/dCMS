...
export function ProductImportPage({ tenantId, onBack }: Props) {
  ...
  async function handleUpload() {
    ...
    setUploading(true);
    try {
      const r = await uploadImport(tenantId, "products", selectedFile);
      setJobId(r.jobId);
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-outline-variant/15 bg-surface px-6 py-4">
        <div>
          ...
          <p className="mt-1 text-sm text-on-surface-variant">
            Bulk add or update products from a spreadsheet file. Files are processed in the background.
          </p>
        </div>
      </div>
        ...
        <section className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-6 shadow-sm">
          ...
          <div className="mt-5 flex items-start gap-3 rounded-md border border-outline-variant/10 bg-surface-container-low p-3">
            ...
            <p className="text-[11px] text-on-surface-variant leading-relaxed">
              Required columns: <code className="rounded bg-outline-variant/20 px-1">sku</code>, <code className="rounded bg-outline-variant/20 px-1">name</code>, <code className="rounded bg-outline-variant/20 px-1">category_id</code>, <code className="rounded bg-outline-variant/20 px-1">price</code>, <code className="rounded bg-outline-variant/20 px-1">store_id</code>. Optional: <code className="rounded bg-outline-variant/20 px-1">description</code>, <code className="rounded bg-outline-variant/20 px-1">slug</code>, <code className="rounded bg-outline-variant/20 px-1">status</code>.
            </p>
          </div>
        </section>
