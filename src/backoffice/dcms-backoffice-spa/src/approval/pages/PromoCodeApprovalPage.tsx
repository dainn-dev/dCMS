...
export function PromoCodeApprovalPage({ tenantId, authToken, onPendingCountChange }: Props) {
  const [rows, setRows] = useState<PromoCodeApprovalRow[]>([]);
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      setRows([]);
      setLoadError(e instanceof Error ? e.message : "Failed to load pending promo codes");
      onPendingCountChange?.(0);
    } finally {
      setLoading(false);
    }
  ...
  const applyApprove = useCallback(
    async (ids: string[]) => {
      ...
      try {
        await Promise.all(ids.map((id) => approvePromoCode(tenantId, id, authToken)));
        setToast(ids.length > 1 ? `Approved ${ids.length} promo codes.` : "Approved.");
        await loadList();
      } catch (e: unknown) {
        setRows(prev);
  ...
  const applyReject = useCallback(
    async (ids: string[], reason: string) => {
      ...
      try {
        await Promise.all(ids.map((id) => rejectPromoCode(tenantId, id, reason, authToken)));
  ...
  const onViewPage = useCallback((row: PromoCodeApprovalRow) => {
    setToast(`Opening storefront promo “${row.promoCode}”.`);
  }, []);
  ...
  const onPreview = useCallback((row: PromoCodeApprovalRow) => {
    ...
    setToast(`Promo preview opened in a new tab — ${row.promoCode}.`);
  }, []);
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 max-w-2xl text-sm text-on-surface-variant">
              Review pending promo codes (spec 6.4). Approve or reject with a required reason.
            </p>
          </div>
        </div>
      </header>
