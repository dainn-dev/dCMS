...
namespace dCMS.Catalog.Api.Campaigns;

/// <summary>
/// DAI-598: Campaign CRUD + workflow transitions + history.
...
public static class CampaignRoutes
{
    ...
    private static async Task<IResult> GetCampaign(
        string tenantId, string id, ICampaignPersistence campaigns,
        CancellationToken cancellationToken = default)
    {
    ...
    private static async Task<IResult> CreateCampaign(
        string tenantId,
        [FromBody] CampaignWriteRequest body,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!CampaignRow.IsValidCode(code))
            return ApiEnvelope.Error("validation_error",
                $"Code '{code}' is invalid. Use uppercase letters, digits, underscores (e.g. SPRING26).",
                StatusCodes.Status400BadRequest);

        if (!CampaignRow.ValidEditorKinds.Contains(body.EditorKind))
            return ApiEnvelope.Error("validation_error",
                $"EditorKind '{body.EditorKind}' is invalid.", StatusCodes.Status400BadRequest);

        if (!CampaignRow.ValidChannels.Contains(body.Channel))
            return ApiEnvelope.Error("validation_error",
                $"Channel '{body.Channel}' is invalid.", StatusCodes.Status400BadRequest);
        ...
        return Results.Json(
            new { data = ToDto(row), meta = (object?)null, error = (object?)null },
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (code != existing.Code &&
            await campaigns.CampaignCodeExistsAsync(tenantId, code, id, cancellationToken).ConfigureAwait(false))
            return ApiEnvelope.Error("conflict", $"Campaign code '{code}' already exists.", StatusCodes.Status409Conflict);
        ...
        var updated = existing with
        {
            ...
            ActiveDaysJson = body.ActiveDaysJson,
            ActiveMonthsJson = body.ActiveMonthsJson,
            QualifiersJson = body.QualifiersJson,
            MechanicsJson = body.MechanicsJson,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (existing.WorkflowState == "active")
            return ApiEnvelope.Error("conflict",
                "Cannot delete an active campaign. Pause or archive it first.",
    ...
    private static async Task<IResult> DoTransition(
        string tenantId, string id, string toState,
        WorkflowActionRequest? body, ICampaignPersistence campaigns,
        HttpContext ctx, CancellationToken cancellationToken)
    {
        try
        {
            var ok = await campaigns.TransitionWorkflowAsync(
                id, tenantId, toState, ActorId(ctx), body?.Comment ?? "", cancellationToken)
        ...
        var updated = await campaigns.GetCampaignAsync(id, tenantId, cancellationToken).ConfigureAwait(false);
        return ApiEnvelope.Ok(ToDto(updated!));
    }

    private static Task<IResult> Submit(string tenantId, string id,
        [FromBody] WorkflowActionRequest? body, ICampaignPersistence campaigns,
        HttpContext ctx, CancellationToken ct)
        => DoTransition(tenantId, id, "pending_approval", body, campaigns, ctx, ct);

    private static Task<IResult> Approve(string tenantId, string id,
        [FromBody] WorkflowActionRequest? body, ICampaignPersistence campaigns,
        HttpContext ctx, CancellationToken ct)
        => DoTransition(tenantId, id, "approved", body, campaigns, ctx, ct);

    private static Task<IResult> Reject(string tenantId, string id,
        [FromBody] WorkflowActionRequest? body, ICampaignPersistence campaigns,
        HttpContext ctx, CancellationToken ct)
        => DoTransition(tenantId, id, "rejected", body, campaigns, ctx, ct);

    private static Task<IResult> Activate(string tenantId, string id,
        [FromBody] WorkflowActionRequest? body, ICampaignPersistence campaigns,
        HttpContext ctx, CancellationToken ct)
        => DoTransition(tenantId, id, "active", body, campaigns, ctx, ct);

    private static Task<IResult> Pause(string tenantId, string id,
        [FromBody] WorkflowActionRequest? body, ICampaignPersistence campaigns,
        HttpContext ctx, CancellationToken ct)
        => DoTransition(tenantId, id, "deactivated", body, campaigns, ctx, ct);

    private static Task<IResult> Archive(string tenantId, string id,
        [FromBody] WorkflowActionRequest? body, ICampaignPersistence campaigns,
        HttpContext ctx, CancellationToken ct)
        => DoTransition(tenantId, id, "archived", body, campaigns, ctx, ct);
}
