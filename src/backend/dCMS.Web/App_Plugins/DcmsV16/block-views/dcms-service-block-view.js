...
export default class DcmsServiceBlockView extends DcmsBlockBase {
  render() {
    ...
    // cornerRadius for both card and pictures (design.cornerRadius.cards / .picturesAndVideo)
    const cardRadius = c.cornerRadius ?? null;
    const picRadius = c.pictureCornerRadius ?? cardRadius ?? null;
    const cardStyle = `${cardRadius !== null ? `border-radius:${cardRadius}px;` : "border-radius:6px;"}box-shadow:0 2px 8px rgba(0,0,0,0.08);`;
  ...
  static styles = [UmbTextStyles, css`
    ...
    .card-media {
      ...
      color: var(--uui-color-text-alt,#767676); gap: 4px; margin: 8px; overflow: hidden;
    }
...
customElements.define("dcms-service-block-view", DcmsServiceBlockView);
