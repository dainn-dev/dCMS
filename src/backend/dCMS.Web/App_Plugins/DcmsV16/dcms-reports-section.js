...
const SPA_JS = `/App_Plugins/DcmsV16/dist/reports-spa.js?v=${_v}`;
const SPA_CSS = `/App_Plugins/DcmsV16/dist/reports-spa.css?v=${_v}`;
...
export default class DcmsReportsSectionElement extends UmbElementMixin(HTMLElement) {
  ...
  async connectedCallback() {
    ...
    try {
      ...
      style.id = "dcms-reports-spa-style";
      ...
      if (this.#host) {
        this.#host.innerHTML =
          `<div style="padding:16px 20px;font-family:system-ui,sans-serif;color:#b91c1c">
            <div style="font-weight:600;margin-bottom:6px">Could not load Reports SPA bundle.</div>
            ...
            <div style="margin-top:10px;font-size:12px;color:#64748b">
              Run <code>npm run build:reports</code> in <code>src/backoffice/dcms-backoffice-spa</code> and restart Umbraco.
            </div>
          </div>`;
      }
      // eslint-disable-next-line no-console
      console.error("[dcms-reports-section]", e);
    }
  }
...
customElements.define("dcms-reports-section", DcmsReportsSectionElement);
