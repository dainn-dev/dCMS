...
function parseValue(raw) {
  try {
    ...
    if (obj && typeof obj === "object") {
      const items = Array.isArray(obj.items) ? obj.items.map(mergeScheme) : [];
      const sel = typeof obj.selected === "number" && obj.selected >= 0 && obj.selected < items.length
        ? obj.selected : null;
...
export default class DcmsColorSchemePickerPropertyEditor extends LitElement {
  static properties = {
    value: { type: String },
    _cv: { state: true },
  ...
  async #mountOrUpdate(hex) {
    ...
    if (!this.shadowRoot?.querySelector("link#dcms-acp-csp")) {
      const link = document.createElement("link");
      link.id = "dcms-acp-csp"; link.rel = "stylesheet"; link.href = ACP_CSS;
  ...
  #addScheme() {
    const items = [...this._cv.items, cloneScheme(DEFAULT_SCHEME)];
    this._cv = { ...this._cv, items };
    this._editIdx = items.length - 1;
    this._editOpen = new Set(["content"]);
    this._active = null;
    this.#teardown();
    this.#commit();
  ...
  #copyScheme(/** @type {number} */ idx) {
    const items = [
      ...this._cv.items.slice(0, idx + 1),
      cloneScheme(this._cv.items[idx]),
      ...this._cv.items.slice(idx + 1),
    ];
    let selected = this._cv.selected;
    if (selected !== null && selected > idx) selected++;
    this._cv = { selected, items };
    this.#commit();
  }

  #deleteScheme(/** @type {number} */ idx) {
    const items = this._cv.items.filter((_, i) => i !== idx);
    let selected = this._cv.selected;
    if (selected === idx) selected = null;
    else if (selected !== null && selected > idx) selected--;
  ...
  #selectScheme(/** @type {number} */ idx) {
    this._cv = { ...this._cv, selected: idx };
    this.#commit();
  }
  ...
  #applyColor(/** @type {string} */ section, /** @type {string} */ key, /** @type {string} */ hex) {
    ...
    const items = this._cv.items.map((item, i) =>
  ...
  #activateField(/** @type {string} */ section, /** @type {string} */ key, /** @type {string} */ label, /** @type {MouseEvent} */ e) {
    ...
    const item = this._cv.items[this._editIdx];
  ...
  static styles = [
    ...
    css`
      ...
      .scheme-card.is-selected {
        border-color: #3cab3b;
        box-shadow: 0 0 0 3px rgba(60,171,59,.2);
      }
      .card-inner {
  ...
  #renderCard(/** @type {any} */ item, /** @type {number} */ idx) {
    const isSelected = this._cv.selected === idx;
    const isEditing = this._editIdx === idx;
    return html`
      <div
        class="scheme-card ${isSelected ? "is-selected" : ""}"
        @click=${() => this.#selectScheme(idx)}
        title="Style ${idx + 1}"
      >
        ...
        <div class="card-actions" @click=${(/** @type {MouseEvent} */ e) => e.stopPropagation()}>
          <button class="card-action-btn" title="Copy" @click=${(/** @type {MouseEvent} */ e) => { e.stopPropagation(); this.#copyScheme(idx); }}>⧉</button>
          <button class="card-action-btn ${isEditing ? "is-active" : ""}" title="Edit" @click=${(/** @type {MouseEvent} */ e) => { e.stopPropagation(); this.#toggleEdit(idx); }}>✎</button>
          <button class="card-action-btn" title="Delete" @click=${(/** @type {MouseEvent} */ e) => { e.stopPropagation(); this.#deleteScheme(idx); }}>✕</button>
        </div>
      </div>
    `;
  }
    ...
    return html`
      <div class="s-csp">
        <div class="scheme-grid">
          ${this._cv.items.map((item, idx) => this.#renderCard(item, idx))}
        </div>
...
if (!customElements.get("dcms-color-scheme-picker-property-editor")) {
  customElements.define("dcms-color-scheme-picker-property-editor", DcmsColorSchemePickerPropertyEditor);
}
