...
/** Access UI lives in the eStore SPA bundle; default to Users when hash is empty. */
const DEFAULT_ACCESS_HASH = "#access-users";
...
export default class DcmsAccessSectionElement extends UmbElementMixin(HTMLElement) {
  ...
  async connectedCallback() {
    ...
    if (!rawHash || rawHash === "/") {
      window.location.hash = DEFAULT_ACCESS_HASH;
    }
    ...
    try {
      ...
      style.id = "dcms-access-spa-style";
      ...
      if (this.#host) {
        this.#host.innerHTML =
          `<div style="padding:16px 20px;font-family:system-ui,sans-serif;color:#b91c1c">
            <div style="font-weight:600;margin-bottom:6px">Could not load Access SPA bundle.</div>
            ...
            <div style="margin-top:10px;font-size:12px;color:#64748b">
              Run <code>npm run build:estore</code> in <code>src/backoffice/dcms-backoffice-spa</code> and restart Umbraco.
            </div>
          </div>`;
      }
      // eslint-disable-next-line no-console
      console.error("[dcms-access-section]", e);
    }
  }
...
customElements.define("dcms-access-section", DcmsAccessSectionElement);
